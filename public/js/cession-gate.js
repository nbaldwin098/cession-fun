(function () {
  ...
  function step(name) {
    ...
    if (name === 'user') {
      root.innerHTML =
        ...
        '<label class="gate-agree"><input id="gateAgree" type="checkbox"> I agree that my trading stats may be public under this username. It cannot be changed.</label>' +
    ...
    if (name === 'wallet') {
      root.innerHTML =
        ...
        '<p>Connect a wallet you have, or make a real BIP-39 phrase here and import it.</p>' +
  ...
  function land() {
    ...
    if (!g) { g = document.createElement('div'); g.id = 'cessionGate'; document.body.appendChild(g); }
    g.className = 'open';
    hideApp(true);
    g.innerHTML =
      ...
      '<img class="gate-word" src="brand/cession-wordmark-white.svg" alt="Cession"></div></div>';
    ...
    btn.onclick = async function () {
      if (mode === 'sign') { mode = 'code'; btn.textContent = 'Enter Code'; wrap.classList.add('on'); input.focus(); return; }
      err.textContent = '';
      try {
  ...
  async function boot() {
    ...
    try {
      ...
      const d = await r.json();
      if (d.open && already()) { localStorage.setItem('cession_gate_ok', '1'); return; }
      if (d.open && !already()) { land(); step('user'); return; }
    } catch (e) {}
    land();
  }
